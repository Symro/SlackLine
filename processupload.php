...
if(isset($_SESSION['membre_logged_in'])){
	if(isset($_POST))
	{
		...
		$ThumbPrefix			= "_original"; //Normal thumb Prefix
		...
		switch(strtolower($ImageType))
		{
			...
			default:
				...
				die(json_encode(
					array(
						"erreur" => true,
						"msg" => "Votre image doit être au format .jpg"
				)));
		}
		...
		$NewImageName = $_SESSION['membre_id'].'.'.$ImageExt;
		//set the Destination Image
		$thumb_DestRandImageName 	= $DestinationDirectory.$NewImageName; //Thumb name
		$DestRandImageName 			= $DestinationDirectory.$_SESSION['membre_id'].$ThumbPrefix.'.'.$ImageExt; //Name for Big Image
		...
		if(resizeImage($CurWidth,$CurHeight,$BigImageMaxSize,$DestRandImageName,$CreatedImage,$Quality,$ImageType))
		{
			...
			if(!cropImage($CurWidth,$CurHeight,$ThumbSquareSize,$thumb_DestRandImageName,$CreatedImage,$Quality,$ImageType))
				{
					...
					die(json_encode(
						array(
							"erreur" => true,
							"msg" => "Erreur lors de la création de l'image"
					)));
				}
			/*
			...
			*/
			...
			die(json_encode(
				array(
					"erreur" => true,
					"msg" => "Erreur lors du redimensionnement"
			)));
		}
	}

}else{
	die('Problème d\'identification');
}
